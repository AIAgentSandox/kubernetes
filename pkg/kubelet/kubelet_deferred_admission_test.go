...
func TestDeferredAdmissionTracksDeferredPod(t *testing.T) {
	...
	require.True(t, kl.IsPodAdmissionDeferred(pod.UID), "deferred pod should be tracked")
...
func TestDeferredAdmissionRetrySucceeds(t *testing.T) {
	...
	require.False(t, kl.IsPodAdmissionDeferred(pod.UID), "admitted pod should be removed from the deferred map")
...
func TestDeferredAdmissionStillDeferredWithinTimeout(t *testing.T) {
	...
	require.True(t, kl.IsPodAdmissionDeferred(pod.UID), "pod within the timeout should still be deferred")
	if status, found := kl.statusManager.GetPodStatus(pod.UID); found {
...
func TestDeferredAdmissionTimesOut(t *testing.T) {
	...
	require.False(t, kl.IsPodAdmissionDeferred(pod.UID), "timed-out pod should be removed from the deferred map")
...
func TestDeferredAdmissionTerminationRequestedNotAdmitted(t *testing.T) {
	...
	require.False(t, kl.IsPodAdmissionDeferred(pod.UID), "terminating pod should be dropped from the deferred map")
...
func TestDeferredAdmissionClearedOnRemove(t *testing.T) {
	...
	require.False(t, kl.IsPodAdmissionDeferred(pod.UID), "removed pod should be cleared from the deferred map")
}
...
func TestDeferredAdmissionOrphanCleanup(t *testing.T) {
	...
	kl.removeOrphanedDeferredAdmissions(sets.New[types.UID]("some-other-pod"))
...
func TestDeferredAdmissionNonDeferrableRejection(t *testing.T) {
	...
	require.False(t, kl.IsPodAdmissionDeferred(pod.UID), "non-deferrable rejection must not be tracked")
	checkPodStatus(t, kl, pod, v1.PodFailed)
}
